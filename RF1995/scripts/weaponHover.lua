...
function this:init(mod)
	sdlext.addFrameDrawnHook(function()
		if self.prev.tbl and self.weapon.tbl ~= self.prev.tbl then
			for _, v in ipairs(self.weapons[self.prev.type]) do
				v.onUnhover(self.prev.tbl, self.prev.type)
			end
		end
		
		if self.weapon.tbl and self.weapon.tbl ~= self.prev.tbl then
			for _, v in ipairs(self.weapons[self.weapon.type]) do
				v.onHover(self.weapon.tbl, self.weapon.type)
			end
		end
