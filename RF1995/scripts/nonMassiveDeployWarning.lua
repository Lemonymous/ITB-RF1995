...
function this:init(mod)
	self.id = mod.id .."_DeploymentWarning"
	
	self.Warning = {
		Link = self.id.. "_Link",
		IconColor = GL_Color(255, 150, 140, 1),
		TileColor = GL_Color(255, 50, 50, 0.75),
		Tile = "combat/tile_icon/".. self.id .."_drown.png",
		Title = "Dangerous Deployment.",
		Desc = "Non-massive Mechs that cannot fly die in water.",
	}

	Location[self.Warning.Tile] = Point(-27,2)
	TILE_TOOLTIPS[self.Warning.Link] = {self.Warning.Title, self.Warning.Desc}
	modApi:appendAsset("img/".. self.Warning.Tile, mod.resourcePath .."img/combat/tile_icon/tile_drown.png")
